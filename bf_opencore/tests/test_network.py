...
def test_create_empty_network(nwk_authorized_client):
    ...
    response = nwk_authorized_client.post("/api/networks/",
...
def test_create_named_network_unauth(auth_client):
    ...
    response = auth_client.post("/api/networks/",
                                json.dumps({"name": "spam"}),
                                content_type="application/json")
...
def test_create_network_then_cidr(supplied_cidr, resulting_cidr,
                                  nwk_authorized_client):
    """Create a network then add one or more CIDR."""
    response = nwk_authorized_client.post("/api/networks/",
                                          json.dumps({"name": "spam"}),
                                          content_type="application/json")
    assert response.status_code == 201
    response = nwk_authorized_client.patch(response.data["url"],
...
def test_get_cidr(supplied_cidr, resulting_cidr, nwk_authorized_client):
    ...
    response = nwk_authorized_client.patch(response.data["url"],
                                           ...
                                           content_type="application/json")
    assert response.status_code == 200
    response = nwk_authorized_client.get("/api/cidrs/",
                                         content_type="application/json")
    assert response.status_code == 200
...
@pytest.mark.xfail(raises=AssertionError,
                   reason="There's a bug in CIDR validation code.  "
                          "Documented in Github issue #1443.")
...
def test_asset_in_network_old_api(nwk_authorized_client):
    ...
    response = nwk_authorized_client.get(
        f"/api/networks/{network.id}/assets/")
...
def test_asset_in_network_new_api(nwk_authorized_client):
    ...
    network.cidr = ["10.0.0.0/24"]
    response = nwk_authorized_client.get(
        f"/api/assets/?network={network.id}")
...
def test_asset_big_network(nwk_authorized_client):
    ...
    assert len(assets) == 2
    assert {a["ip_address"] for a in assets} == set(["192.168.218.101",
                                                     "192.168.218.102"])
    response_big = nwk_authorized_client.get(
        f"/api/assets/?network={network_big.id}")
