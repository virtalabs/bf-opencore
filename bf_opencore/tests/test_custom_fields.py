...
def test_custom_field_via_model(cleandb, auth_client):
    ...
    assert (asset['asset_custom_fields'][0]['field']['id'] ==
            custom_field_name.id)
...
def test_api_add_custom_field_via_asset(cleandb, auth_client, admin_client):
    ...
    res = admin_client.post('/api/assets/{}/customfields/'.format(asset_id),
                            **kwargs)
...
def test_api_change_custom_field(cleandb, auth_client, admin_client):
    ...
    res = admin_client.patch(
        '/api/assetcustomfields/{}/'.format(custom_field['id']),
        **kwargs)
...
def test_api_delete_custom_field(cleandb, auth_client, admin_client):
    ...
    res = admin_client.delete('/api/assetcustomfields/{}/'
                              ''.format(custom_field['id']))
...
def test_api_field_unauthorized_patch(cfield, auth_client):
    ...
    kwargs = {'data': json.dumps({'value_text': 'very shiny'}),
              'content_type': 'application/json'}
    res = auth_client.patch('/api/assetcustomfields/{}/'
                            ''.format(cfield_id), **kwargs)
    assert res.status_code == 403
...
def test_api_field_authorized_patch(cfield, asset_edit_client):
    ...
    kwargs = {'data': json.dumps({'value_text': 'very shiny'}),
              'content_type': 'application/json'}
    res = asset_edit_client.patch('/api/assetcustomfields/{}/'
                                  ''.format(cfield_id), **kwargs)
    assert res.status_code == 200
...
def test_api_field_admin_patch(cfield, admin_client):
    ...
    kwargs = {'data': json.dumps({'value_text': 'very shiny'}),
              'content_type': 'application/json'}
    res = admin_client.patch('/api/assetcustomfields/{}/'
                             ''.format(cfield_id), **kwargs)
    assert res.status_code == 200
...
def test_api_field_unauthorized_delete(cfield, auth_client):
    ...
    kwargs = {'data': json.dumps({'value_text': 'very shiny'}),
              'content_type': 'application/json'}
    res = auth_client.delete('/api/assetcustomfields/{}/'
                             ''.format(cfield_id), **kwargs)
    assert res.status_code == 403
...
def test_api_field_authorized_delete(cfield, asset_edit_client):
    ...
    kwargs = {'data': json.dumps({'value_text': 'very shiny'}),
              'content_type': 'application/json'}
    res = asset_edit_client.delete('/api/assetcustomfields/{}/'
                                   ''.format(cfield_id), **kwargs)
    assert res.status_code == 204
