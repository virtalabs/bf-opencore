...
class TestAssetHistory:
    ...
    def test_history_simple_change(self, asset_edit_client):
        ...
        res = asset_edit_client.patch(f"/api/assets/{self.spam_id}/",
...
class TestOneFieldHistory:
    ...
    def setup_history(self, asset_edit_client):
        ...
        for new_data in self.change_sequence[2:]:
            _ = client.patch(f"/api/assets/{self.asset_id}/",
    ...
    def test_history_default_null(self, asset_edit_client):
        ...
        res = asset_edit_client.get(f"/api/assets/{self.asset_id}/history/?field={default_null_field}")
        assert res.status_code == 200
    ...
    def test_history_default_blank(self, asset_edit_client):
        ...
        res = asset_edit_client.get(f"/api/assets/{self.asset_id}/history/?field={non_null_field}")
        assert res.status_code == 200
        rj = res.data
        assert len(rj) == 2
...
class TestAssetRiskHistory:
    ...
    def test_risk_history(self, auth_client):
        res = auth_client.get(
            f"/api/assets/{self.asset_id}/history/?field=risk_score")
        assert res.status_code == 200
