...
class TestAssetHistory():
    ...
    def setup_assets(self, db):
        ...
        for hostname in asset_names:
            self.asset_record[hostname] = bf_mod.Asset.objects.create(
                hostname=hostname)
    ...
    def test_spam_id_2(self, auth_client):
        res = auth_client.get('/api/assets/{}/'.format(self.spam_id))
        assert res.status_code == 200
        assert res.data['id'] == self.spam_id

    def test_spam_simple_history(self, auth_client):
        res = auth_client.get('/api/assets/{}/history/'.format(self.spam_id))
        assert res.status_code == 200
    ...
    def test_history_simple_change(self, asset_edit_client):
        ...
        res = asset_edit_client.patch('/api/assets/{}/'.format(self.spam_id),
                                      json.dumps({'hostname': 'nospam'}),
                                      content_type='application/json')
        assert res.status_code == 200
        res = asset_edit_client.get('/api/assets/{}/history/'
                                    ''.format(self.spam_id))
    ...
    def test_history_change_order(self, asset_edit_client):
        ...
        hostname_history = [(hi['hostname'], hi['risk_score'])
                            for hi in res.data['results']]
        # Extra 'spam' at the end due to initial rescore on asset creation
        assert hostname_history == [('spam_2', 0.0),
                                    ('spam_1', 0.0),
                                    ('spam', 0.0),
                                    ('spam', None)]
    ...
    def test_changelog_change_order(self, asset_edit_client):
        ...
        assert hostname_changelog == [('spam_2', None),
                                      ('spam_1', None),
                                      (None, 0.0),
                                      ('spam', None)]
    ...
    def test_spam_simple_changelog(self, auth_client):
        ...
        res = auth_client.get('/api/assets/{}/changelog/'.format(self.spam_id))
        assert res.status_code == 200
...
class TestOneFieldHistory():
    ...
    def setup_history(self, asset_edit_client):
        ...
        self.history_keys = ('hostname', 'serial_number',
                             'manufacturer', 'model',)
        self.history_data = [
            ('Wile', None, None, None),
            ('Wile', None, None, None),
        ...
        for new_data in self.change_sequence[2:]:
            _ = client.patch('/api/assets/{}/'.format(self.asset_id),
    ...
    def test_history_newestfirst(self, asset_edit_client):
        ...
        for hist_item, api_hist_item in zip(reversed(self.history_data),
                                            res.data['results']):
            hist_dict = {k: hist_item[i]
                         for i, k in enumerate(self.history_keys)}
            api_hist_dict = {k: api_hist_item[k]
                             for k in self.history_keys}
            assert hist_dict == api_hist_dict

    def test_changelog(self, asset_edit_client):
        """Changelog should be newest-first."""
        res = asset_edit_client.get('/api/assets/{}/changelog/'.format(
            self.asset_id))
        # NOTE: the changelog data as seen on the API is newest-first
        for hist_item, api_hist_item in zip(reversed(self.changelog_data),
                                            res.data):
            hist_dict = {k: hist_item[i]
                         for i, k in enumerate(self.history_keys)}
            api_hist_dict = {k: api_hist_item[k]
    ...
    def test_one_field_history(self, asset_edit_client):
        ...
        res = asset_edit_client.get(
            '/api/assets/{}/history/?field=hostname'.format(self.asset_id))
        # This gives us the history of one particular field.
        # NOTE: the history data as seen on the API is newest-first
        for hist_hostname, api_hist_item in (
                zip(reversed(self.hostname_history),
                    res.data)):
    ...
    def test_history_default_null(self, asset_edit_client):
        ...
        res = asset_edit_client.get('/api/assets/{}/history/?field={}'.format(
            self.asset_id, default_null_field))
        assert res.status_code == 200
...
@pytest.mark.parametrize('field_name, change_sequence',
                         [('category', ['foo']),
                          ('category', ['']),
                          ('category', ['foo', 'bar']),
                          ('category', ['', 'foo']),
...
def test_history_empty_values(asset_edit_client, field_name, change_sequence):
    ...
    create_json = json.dumps({'hostname': 'real_hostname',
                              field_name: change_sequence[0]})
    # update_list is a list of json objects/"dicts"
    update_list = [json.dumps({field_name: changeval})
                   for changeval in change_sequence[1:]]
    # Create the asset, then patch it
    resp = asset_edit_client.post('/api/assets/', create_json,
                                  content_type='application/json')
    # asset_url is on the form '/api/assets/<n>/'
    asset_url = resp.data["url"]
    for update_json in update_list:
        asset_edit_client.patch(asset_url, update_json,
                                content_type='application/json')
    ...
    api_change_sequence = [hist_item[field_name]
                           for hist_item in reversed(res.data)]
...
class TestAssetRiskHistory():
    ...
    def test_history_complete(self, auth_client):
        res = auth_client.get('/api/assets/{}/history/'.format(self.asset_id))
    ...
    def test_risk_history(self, auth_client):
        res = auth_client.get(
            '/api/assets/{}/history/?field=risk_score'.format(self.asset_id))
        assert res.status_code == 200
...
def test_risk_factor_history(admin_client):
    ...
    resp = admin_client.post('/api/riskfactors/',
                             json.dumps(params),
                             content_type='application/json')
