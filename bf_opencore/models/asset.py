...
from bf_opencore.bf_opencore.utils import NullUnlessChanged
...
from bf_opencore.connectors.models import ConnectorTask
...
class Asset(models.Model):
    ...
    def display_name(self):
        ...
        if self.name:
            ...
            if self.model:
                d_name = '{}-{}-{}'.format(
                    self.manufacturer, self.model, self.id)
            else:
                d_name = '{}-{}'.format(self.manufacturer, self.id)
        elif self.nic_vendor:
            d_name = '{}-{}'.format(self.nic_vendor, self.id)
        else:
            d_name = "Asset-{}".format(self.id)
        return d_name
    ...
    def similar_qset(self, exclude_self=True):
        ...
        if self.mac_address is not None:
            ...
            mac_pfx = ':'.join('{:02x}'.format(s) for s in octets[:3])
            min_mac = '{}:00:00:00'.format(mac_pfx)
            max_mac = '{}:ff:ff:ff'.format(mac_pfx)
    ...
    def add_risk_factor(self, shortname, value, reason=None):
        ...
        try:
            ...
            raise RiskFactor.DoesNotExist(
                'Cannot add unknown risk factor {} to asset {}'
    ...
    def update_or_create_fk_field(self, name, value, reason=None):
        ...
        # Parse name
        assert "__" in name, "Expected '__' in fk field {}".format(name)
        ...
        assert fieldtype == "ForeignKey", (
            "Expected '{}' of '{}' to be a ForeignKey.  Got {}."
            "".format(fk_name, name, fieldtype)
        )
        ...
        if asset_field == "asset_custom_fields":
            ...
            try:
                ...
                raise AssetCustomFieldName.DoesNotExist(
                    'Cannot add unknown custom field {} to asset {}'
                    ''.format(name, self)
                )
            ...
            try:
                ...
                raise ValidationError(
                    "Not a valid risk factor value: {}".format(value))
            arf, _ = self.asset_risk_factors.update_or_create(
                asset=self,
                risk_factor=rfac,
            ...
            assert False, "Unsupported foreign key: '{}'".format(asset_field)
    ...
    def _soft_clip(x, rmax=10, rmin=0, a=1, typ='arctan'):
        ...
        if typ == 'arctan':
            ...
            raise ValueError("'typ' must be 'arctan' or 'inv_x'; was {}"
                             "".format(typ))
        return clipped * rmax
    ...
    def rescore_asset_on_save(cls, sender, instance, created, raw, using,
        ...
        if created or instance.app_sw_version:
            ...
            for asset in instance.similar_qset():
                ...
                setattr(asset, "norecurse", True)  # HACK break cycle
    ...
    def __str__(self):  # noqa
        return "{}:{}:{}".format(self.id, self.display_name, self.ip_address)
