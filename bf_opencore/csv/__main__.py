...
"""Top level interface for CSV connector."""
import os
...
def main(ctx, filename=None, field_mapping=None, no_reset_progress=False,
    ...
    if ctx.ct.attachment:
        ...
        if not os.path.exists(filename):
            raise ConnectorTaskError("Provide an upload file")
...
@click.option("--fieldmap", type=click.Path(exists=True), default=None)
def cli(filename, fieldmap):
    """CSV connector command line interface."""
    logger.setLevel(logging.DEBUG)

    # Parse field map from file if provided
    fieldmap_dict = None
    if fieldmap:
        with open(fieldmap, 'r') as fieldmap_file:
            fieldmap_dict = json.load(fieldmap_file)

    # Run main function
    main.apply(kwargs={'filename': filename, 'field_mapping': fieldmap_dict})
