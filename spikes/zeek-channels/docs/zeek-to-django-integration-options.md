...
- **Third-party `zeek-redis` packages were investigated and ruled out for Stage 1** — no current Zeek package writes to Redis Streams; both `sedarasecurity/zeek-redis` and `mbispham/zeekjs-redis` use Redis Lists; the legacy Bro::Redis is documented as not production-ready by its own author ([bridge-investigation-findings.md](bridge-investigation-findings.md)). The decision was originally to use Vector. **Amended 2026-05-12** ([bridge-decision-revisited.md](bridge-decision-revisited.md)): the Stage 1 prototype bridge is bespoke ZeekJS — a ~30-line in-process producer using Zeek's bundled JavaScript runtime (in-tree since Zeek 6.0). The C-plugin fork (`sedarasecurity/zeek-redis` patched `LPUSH`→`XADD`, ~20-line C++ patch) and Vector are documented swap-in paths; neither changes the consumer. The Zeek 8 Storage Framework KV side-channel is evaluated and explicitly rejected. The bridge layer is swappable behind the stream — the payload contract on `zeek:events` is the integration boundary.
...
1. A bridge mechanism writes each relevant Zeek event to a Redis stream via `XADD zeek:events * stream <name> payload <json>`. The Stage 1 prototype bridge is **bespoke ZeekJS** — a ~30-line in-process producer using Zeek's bundled JavaScript runtime ([bridge-decision-revisited.md](bridge-decision-revisited.md), amended 2026-05-12). The original decision was Vector, fixed by the §6.0 [investigation findings](bridge-investigation-findings.md) that ruled out current zeek-redis packages; the amendment surfaced ZeekJS as a simpler in-process path that doesn't require maintaining a forked plugin. Two swap-in paths are documented and leave the consumer unchanged: (i) a C-plugin fork of `sedarasecurity/zeek-redis` with `LPUSH`→`XADD` patched, for production hardening when Node.js footprint becomes binding; (ii) Vector when disk-buffered durability across Redis outages matters more than in-process simplicity. A Broker subscriber remains a real Stage 3 alternative. Custom Python tailing is excluded from the recommended set per survey §4.2.
...
- **A bridge is still required.** The Stage 1 prototype bridge is **bespoke ZeekJS** in-process — see [bridge-decision-revisited.md](bridge-decision-revisited.md) (amended 2026-05-12; supersedes the original "Vector" answer from [investigation findings](bridge-investigation-findings.md)). The ZeekJS path runs inside the Zeek process, so it doesn't add a daemon; the C-plugin and Vector swap-in paths do. Either way, the "Zeek → Streams" arrow needs an implementation somewhere — that implementation is now a ~30-line JS script rather than a custom daemon.
...
| **Bespoke ZeekJS bridge for Option D** — a ~30-line JS script running inside Zeek (via the bundled JavaScript runtime, in-tree since v6.0) that calls `redis.xAdd('zeek:events', ...)` from a `Conn::log_policy` hook | The Stage 1 prototype default as of the 2026-05-12 amendment. Full analysis in [bridge-decision-revisited.md](bridge-decision-revisited.md). No new daemon; native typed access to Zeek records; fastest iteration. Costs are the Node.js footprint (~50MB) and in-process coupling to Redis. Two documented swap-ins exist for when those costs become binding |
| **C-plugin fork of `sedarasecurity/zeek-redis` as bridge for Option D** — patch `LPUSH`→`XADD` in `src/RedisWriter.cc` (~20-line C++ patch) | Documented as the production-hardening swap-in path from ZeekJS when Node.js footprint or in-process coupling becomes unacceptable. Smaller runtime footprint (hiredis ~200KB vs. Node ~50MB), slower iteration (CMake rebuild cycle), and we own the fork. See [bridge-decision-revisited.md](bridge-decision-revisited.md) |
| **Vector / Filebeat as bridge for Option D** — Vector tails Zeek logs and `XADD`s to Streams | The original §6.0 conclusion; demoted to a swap-in path by the 2026-05-12 amendment. Distinctive advantage is disk-buffered durability across Redis outages — load-bearing if Redis is remote or unreliable, overkill on a co-located small-hospital sensor. Battle-tested file→stream shipper with retry, backoff, and observability built in; the cost is one more daemon to operate |
| **`zeek-redis` (third-party) plugin as primary bridge for Option D** — Zeek scripts publish to a Redis stream directly via a community plugin | Investigated and ruled out — no current package writes to Redis Streams. See [bridge-investigation-findings.md](bridge-investigation-findings.md). Revisitable if a Streams-supporting community package emerges; in the meantime, the in-process producer paths above (ZeekJS or forked C plugin) cover the architectural slot |
| **Zeek 8 Storage Framework Redis backend as bridge for Option D** — Zeek scripts call `Storage::Sync::put` and a downstream consumer reads the KV store via `SCAN` or keyspace notifications | Evaluated and explicitly rejected (2026-05-12). The Storage Framework is a KV API for Zeek-script-internal state; its downstream-read mechanisms (`GET`, `SCAN`, keyspace pubsub) all cancel Option D's durability properties. See [bridge-decision-revisited.md](bridge-decision-revisited.md) |
...
8. **Bridge mechanism for Option D.** ~~Pending investigation.~~ ~~Resolved 2026-05-08 as Vector.~~ **Amended 2026-05-12** — Stage 1 prototype bridge is bespoke ZeekJS (~30-line in-process producer using Zeek's bundled JavaScript runtime); C-plugin fork of `sedarasecurity/zeek-redis` and Vector are documented swap-in paths; the Zeek 8 Storage Framework KV side-channel is rejected. A Broker subscriber remains a possible Stage 3 alternative if low-latency live ingestion later dominates over file durability. Full revised analysis in [bridge-decision-revisited.md](bridge-decision-revisited.md).
